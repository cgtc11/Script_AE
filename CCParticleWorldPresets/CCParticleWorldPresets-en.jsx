/*
    ...
    - Saves greyed-out items as [UNREADABLE]
*/

(function () {
    app.beginUndoGroup("CC Particle World Preset Manager");

    // ===== Property tails to be treated as integers =====
    var integerProps = {
    ...
    // ===== Default preset: "Sparks" (complete) =====
    // NOTE: Keys reflect the effect's property names in your AE locale.
    // Japanese keys below are kept intentionally for compatibility.
    var defaultPreset = {
    ...
    // ===== Recursive collect (save greyed-out too) =====
    ...
    function saveToDisk() {
        ...
        } catch(e) { alert("Save failed: " + e.toString()); }
    }
    ...
    function loadFromDisk() {
        ...
        } catch(e) { alert("Load error: " + e.toString()); }
    }
    ...
    var btnApply  = grp.add("button", undefined, "Apply");
    var btnSave   = grp.add("button", undefined, "Save Current");
    var btnDelete = grp.add("button", undefined, "Delete");
    var btnRename = grp.add("button", undefined, "Rename");

    var grp2 = win.add("group"); grp2.orientation = "row";
    var btnExport = grp2.add("button", undefined, "Export (.TXT)");
    var btnImport = grp2.add("button", undefined, "Import (.TXT)");

    var grp3 = win.add("group"); grp3.orientation = "row";
    var lblPath = grp3.add("statictext", undefined, "Save to: " + presetFile.fsName, {truncate:"middle"});
    lblPath.preferredSize.width = 420;
    var btnChangePath = grp3.add("button", undefined, "Change Save Location");
    ...
    function refreshList() {
        ...
        lblPath.text = "Save to: " + presetFile.fsName;
    }

    btnChangePath.onClick = function () {
        var f = Folder.selectDialog("Choose a folder to save presets");
        if (f) { presetFile = new File(f.fsName + "/CCParticleWorldPresets.txt"); saveToDisk(); refreshList(); }
    };

    // Core apply (returns true on success)
    function applySetting(preset) {
        var comp = app.project.activeItem;
        if (!(comp instanceof CompItem)) { alert("Please activate a Comp."); return false; }
        if (comp.selectedLayers.length === 0) { alert("Please select a layer."); return false; }
    ...
    btnApply.onClick = function () {
        if (listBox.selection) {
            ...
            if (applySetting(settingsStore[idx].data)) {
                alert(settingsStore[idx].name + " applied!");
            }
        } else {
            if (applySetting(defaultPreset)) {
                alert("Sparks applied!");
            }
        }
    };
    ...
    btnSave.onClick = function () {
        var comp = app.project.activeItem;
        if (!(comp instanceof CompItem)) { alert("Please activate a Comp."); return; }
        if (comp.selectedLayers.length === 0) { alert("Please select a layer."); return; }
        var layer = comp.selectedLayers[0];
        var fx = layer.Effects.property("CC Particle World");
        if (!fx) { alert("CC Particle World is not applied."); return; }

        var name = prompt("Enter a name for this preset","Preset" + (settingsStore.length+1));
        if (!name) return;
    ...
    btnRename.onClick = function () {
        if (!listBox.selection) return;
        var idx = listBox.selection.index;
        var newName = prompt("Enter a new name", settingsStore[idx].name);
        if (!newName) return;
    ...
    btnExport.onClick = function () {
        if (!listBox.selection) return;
        var idx = listBox.selection.index;
        var f = File.saveDialog("Choose a file name to export", "*.txt");
        if (!f) return;
        ...
        try {
            ...
            alert("Exported: " + f.fsName);
        } catch(e) { alert("Export failed: " + e.toString()); }
    };

    btnImport.onClick = function () {
        var f = File.openDialog("Select a preset file to import", "*.txt");
        if (!f) return;
        try {
    ...
    if (settingsStore.length === 0) {
        settingsStore.push({name:"Sparks", data:defaultPreset});
        saveToDisk();
    }
    refreshList();
