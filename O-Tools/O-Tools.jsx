//===========================================================================
// O_Tools V1.5.9a by Digimonkey
// Auto language: Japanese AE = Japanese UI / all other languages = English UI
//===========================================================================

var __OToolsIsJapanese = false;
try {
    __OToolsIsJapanese = (String(app.isoLanguage).toLowerCase().indexOf("ja") === 0);
} catch (__OToolsLanguageError) {
    __OToolsIsJapanese = false;
}

function __OToolsText(jaText, enText) {
    return __OToolsIsJapanese ? jaText : enText;
}
...
function buildNameEdUI(panel) {
    ...
    var nameRadio = radioGroup.add("radiobutton", undefined, __OToolsText("名前", "Name"));
    var commentRadio = radioGroup.add("radiobutton", undefined, __OToolsText("コメント", "Comments"));
    ...
    var getButton = buttonGroup.add("button", undefined, __OToolsText("取得", "Get"));
    getButton.helpTip = __OToolsText("選択されたアイテムの名前またはコメントを取得します", "Retrieve the name or comment of the selected item.");

    var updateButton = buttonGroup.add("button", undefined, __OToolsText("更新", "Update"));
    updateButton.helpTip = __OToolsText("テキストフィールドのデータで選択されたアイテムを更新します", "Updates the selected item in the text field data.");
    ...
    var replaceButton = replaceGroup.add("button", undefined, __OToolsText("置換", "Replace"));
    replaceButton.helpTip = __OToolsText("指定されたテキストを新しいテキストに置き換えます", "Replace the specified text with new text.");
...
function buildTReMapUI(panel) {
    ...
    var stopKeysLabel = stopKeysGroup.add("statictext", undefined, __OToolsText("---------コマ抜きアニメーション---------", "-----------Limited animation-----------"));
    stopKeysLabel.helpTip = __OToolsText("コマ抜きアニメーション作成", "Limited Animation Production");
    ...
    var T1_Btn_selecr_Keys = T1_Btn_selecr_Keys_Group.add("button", undefined, __OToolsText("コマ抜き", "Limited Animation"));
    var T1_Btn_selecr_Keys_Num = T1_Btn_selecr_Keys_Group.add("edittext", undefined, "2");

    // ボタンとEditTextのサイズを適切に設定
    T1_Btn_selecr_Keys.helpTip = __OToolsText("コマ抜きアニメーション", "Number");
    T1_Btn_selecr_Keys.size = __OToolsIsJapanese ? [80, 30] : [120, 30];
    T1_Btn_selecr_Keys_Num.helpTip = __OToolsText("抜きたい数", "Number to pull out");
    T1_Btn_selecr_Keys_Num.size = [30, 30];  // EditTextのサイズを適切に調整

    // カーソル合わせで出てくる言葉
            T1_Btn_selecr_Keys.helpTip   = __OToolsText("キーを等間隔で選択します", "Select keys equally spaced");
        ...
        function CHECK_TIMEREMAP_COM(Layer)
        {
            //■「null」を選択していないかどうか
                if(Layer.nullLayer == true){
                    Layer.timeRemapEnabled = false;
                    eval(__OToolsText("throw \"Nullレイヤーが選択されています。\"; ", "throw \"Null layer is selected.\"; "));
                }
            //■「ライト」を選択していないかどうか
                if(Layer instanceof LightLayer){
                    Layer.timeRemapEnabled = false;
                    eval(__OToolsText("throw \"ライトレイヤーが選択されています。\";", "throw \"A light layer is selected.\";"));
                }
            //■「カメラ」を選択していないかどうか
                if(Layer instanceof CameraLayer){
                    Layer.timeRemapEnabled = false;
                    eval(__OToolsText("throw \"カメラレイヤーが選択されています。\";", "throw \"The camera layer is selected.\";"));
                }
            //■「平面」を選択していないかどうか
                if(Layer.source.mainSource instanceof SolidSource){
                    Layer.timeRemapEnabled = false;
                    eval(__OToolsText("throw \"平面レイヤーが選択されています。\";", "throw \"A planar layer is selected.\";"));
                }
        }
    ...
    //■「全フレームに停止キー」ボタン押下時の処理(T1_Btn_Stop_All)
        function ALL_KEYFRAME_STOP()
        {
            try{
                ...
                alert(err_message,__OToolsText("エラー", "error"));
            }
        }
    ...
    //■「セレクトキーフレーム」ボタン押下時の処理(T1_Btn_IR)
        function SELECT_KEYS()
        {
            ...
            } catch(err_message){ alert(err_message,__OToolsText("エラー", "error")); }
        }
...
var T1_Btn_offset = T1_Btn_offset_Group.add("button", undefined, __OToolsText("オフセット", "offset"));
T1_Btn_offset.size = [80, 30];
var T1_Btn_offset_Num = T1_Btn_offset_Group.add("edittext", undefined, "0");
T1_Btn_offset_Num.size = [30, 30];
T1_Btn_offset.helpTip = __OToolsText("選択キーの値を指定フレーム数だけずらす", "Shift the value of the selection key by the specified number of frames");
...
T1_Btn_offset.onClick = function () {
    ...
    var comp = app.project.activeItem;
    if (!(comp && comp instanceof CompItem)) {
        alert(__OToolsText("コンポジションを選択してください。", "Please select a composition."));
    ...
    if (isNaN(frames)) {
        alert(__OToolsText("数値を入力してください", "Please enter a number."));
        app.endUndoGroup(); return;
    }
...
var T1_Btn_spacing = T1_Btn_spacing_Group.add("button", undefined, __OToolsText("間隔整列", "Space Align"));
...
T1_Btn_spacing.helpTip = __OToolsText("0ならフル、1なら1コマ飛ばし、2なら2コマ飛ばしで配置します", "0 = full, 1 = place every other frame, 2 = place every 2 frames");
...
T1_Btn_spacing.onClick = function () {
    ...
    var stopKeysLabel = stopKeysGroup.add("statictext", undefined, __OToolsText("---------タイムリマップ用---------", "--------- For Time Remapping ---------"));
    var w = new Window("palette", __OToolsText("LOOP設定", "LOOP Settings"));
    ...
    btnFreezeFirst.helpTip = __OToolsText("最初の1枚目でLOOP再生", "LOOP playback on the first frame");
    ...
    btnFreezeLast.helpTip = __OToolsText("再生後最後の1枚目でLOOP再生", "LOOP playback on the last frame after playback");
    ...
    btnLoop.helpTip = __OToolsText("全体をLOOP再生（コンポ／連番フッテージ）", "Loop playback (Comp / Image Sequence)");
    btnLoop.size = [80, 30];
    btnLoop.onClick = function () {
        app.beginUndoGroup(__OToolsText("LoopOut設定", "LoopOut Setting"));
        var comp = app.project.activeItem;
    ...
    btnLoopMarkers.helpTip = __OToolsText("LOOPマーカー利用", "Loop using marker");
    ...
    btnLoopMarkers.onClick = function () {
        ...
        for (var i=0;i<layers.length;i++){
            ...
            if (!markerProp || markerProp.numKeys===0){ alert(__OToolsText("マーカーなし: ", "No marker: ")+ly.name); continue; }
            ...
            if (L===null){ alert(__OToolsText("“LOOP”系マーカー未検出: ", "No 'LOOP' marker found: ")+ly.name); continue; }
...
// ★★★★★選択されたレイヤーの順序を逆にする（選択範囲内）★★★★★
    ...
    var reverseLabel = reverseGroup.add("statictext", undefined, __OToolsText("-------選択レイヤーを逆配置-------", "------- Reverse Layer Order -------"));
    reverseLabel.helpTip = __OToolsText("選択されたレイヤーの順序を逆にします", "Reverses the order of the selected layers");

    var reverseButton = reverseGroup.add("button", undefined, __OToolsText("逆配置", "Reverse Layers"));
    reverseButton.helpTip = __OToolsText("選択範囲内でレイヤーの順序を反転させます", "Reverses the order of layers within the selected range");
    reverseButton.size = __OToolsIsJapanese ? [120, 30] : [150, 30];
    ...
    function reverseLayers() {
        var comp = app.project.activeItem;
        if (!(comp && comp instanceof CompItem)) {
            alert(__OToolsText("アクティブなコンポジションを選択してください。", "Please select an active composition."));
            return;
        }

        var selectedLayers = comp.selectedLayers;
        if (selectedLayers.length < 2) {
            alert(__OToolsText("少なくとも2つのレイヤーを選択してください。", "Please select at least two layers."));
            return;
        }

        app.beginUndoGroup(__OToolsText("選択範囲内で順序を反転", "Reverse Selected Layers Order"));
    ...
    var assignCommentLabel = assignCommentGroup.add("statictext", undefined, __OToolsText("-------コメントをテキストに変換------", "--------Assign Comments to Text--------"));
    assignCommentLabel.helpTip = __OToolsText("レイヤーのコメントをテキストに変換します", "Converts layer comments to text");

    var assignCommentButton = assignCommentGroup.add("button", undefined, __OToolsText("文字変換", "Convert Comments to Text"));
    assignCommentButton.helpTip = __OToolsText("選択されたレイヤーのコメントをテキストに変換", "Convert comments on selected layers to text");
...
function convertCommentsToTextAndClearNames() {
    ...
    if (comp != null && comp instanceof CompItem) {
        ...
        if (selectedLayers.length > 0) {
            ...
            alert(__OToolsText("テキストレイヤーを選択してください。", "Please select one or more text layers."));
        }
    } else {
        alert(__OToolsText("コンポジションが選択されていません。", "No composition is selected."));
    }
}
    ...
    var adjustTimeLabel = assignCommentGroup.add("statictext", undefined, __OToolsText("-------文字に応じて時間調整-------", "------------Adjust Duration------------"));
    adjustTimeLabel.helpTip = __OToolsText("文字数に応じてレイヤーの時間を調整", "Layer times based on number of characters");

    var adjustTimeButton = adjustTimeGroup.add("button", undefined, __OToolsText("時間調整", "Adjust Duration"));
    adjustTimeButton.helpTip = __OToolsText("1文字を指定nのフレームとしてレイヤーの時間を調整します", "Adjusts the time of the layer with one character as the specified n frames.");
    adjustTimeButton.size = __OToolsIsJapanese ? [80, 30] : [120, 30];  // English label is wider

    var frameInput = adjustTimeGroup.add("edittext", undefined, "5");
    frameInput.helpTip = __OToolsText("1文字あたりのフレーム数を入力します", "Enter the number of frames per character");
    ...
    adjustTimeButton.onClick = function () {
        ...
        if (!isNaN(frameDuration) && frameDuration > 0) {
            ...
            alert(__OToolsText("有効な数値を入力してください。", "Please enter a valid number."));
        }
...
// ★★★★★レイヤーの長さに合わせてコンポ時間を調整（選択優先）★★★★★
    ...
    var adjustCompDurationLabel = adjustCompDurationGroup.add("statictext", undefined, __OToolsText("--レイヤー長さに基づくコンポ調整--", "-- Adjust Comp to Layer Length --"));
    adjustCompDurationLabel.helpTip = __OToolsText("選択レイヤー（未選択なら全レイヤー）に合わせてコンポ時間を調整", "Adjust comp duration based on selected layers (or all layers if none selected)");

    var adjustCompDurationButton = adjustCompDurationGroup.add("button", undefined, __OToolsText("コンポ時間調整", "Adjust Comp Duration"));
    adjustCompDurationButton.helpTip = __OToolsText("レイヤーの範囲に基づいてコンポジションの時間を調整します", "Adjusts the composition duration based on the range of the layers");
    adjustCompDurationButton.size = __OToolsIsJapanese ? [120, 30] : [150, 30];
    ...
    function adjustCompDuration() {
        ...
        if (targetLayers.length === 0 || comp.numLayers === 0) {
            alert(__OToolsText("レイヤーが存在しません。", "No layers found."));
            return;
        }

        app.beginUndoGroup(__OToolsText("レイヤーに基づいてコンポ時間を調整", "Adjust Comp Duration Based on Layers"));
    ...
    var moveToWorkAreaLabel = moveToWorkAreaGroup.add("statictext", undefined, __OToolsText("---------作業エリア先頭へ移動---------", "---------Move to work area---------"));
    moveToWorkAreaLabel.helpTip = __OToolsText("選択したレイヤーを作業領域に移動します", "Moves the selected layer to the start time of the work area");

    var moveToWorkAreaButton = moveToWorkAreaGroup.add("button", undefined, __OToolsText("移動", "Move"));
    moveToWorkAreaButton.helpTip = __OToolsText("選択したレイヤーを作業領域に移動します", "Moves the selected layer to the start time of the work area");
...
function moveLayersToWorkArea() {
        ...
        if (comp && comp instanceof CompItem) {
            app.beginUndoGroup(__OToolsText("レイヤーをフルサイズで移動", "Move Layers to Work Area"));
            var workAreaStart = comp.workAreaStart;
            var layers = comp.selectedLayers;

            if (layers.length === 0) {
                alert(__OToolsText("移動するレイヤーを選択してください", "Select the layer to be moved."));
                app.endUndoGroup();
                return;
            }
            ...
            alert(__OToolsText("コンポジションを選択してください", "Select a composition."));
        }
    }
...
// 説明ラベル
var clearNamesLabel = clearNamesGroup.add("statictext", undefined, __OToolsText("-------------名前を初期化-------------", "------------- Initialize Names -------------"));

// チェックボックス
var convertToEnglishCheckBox = clearNamesGroup.add("checkbox", undefined, __OToolsText("カメラ, ライト, シェイプ, ヌルを英語化", "English naming for Cam, Lit, Shape, Null"));
convertToEnglishCheckBox.value = false;

// 初期化ボタン
var clearButton = clearNamesGroup.add("button", undefined, __OToolsText("初期化", "Initialize"));
clearButton.onClick = function () {
    ...
    function getColorName(rgb) {
        ...
        if (r > 230 && g > 230 && b > 230) return __OToolsText("ホワイト", "White");
        if (r < 25 && g < 25 && b < 25) return __OToolsText("ブラック", "Black");
        if (r > 200 && g < 50 && b < 50) return __OToolsText("レッド", "Red");
        if (r < 50 && g > 180 && b < 50) return __OToolsText("グリーン", "Green");
        if (r < 50 && g < 50 && b > 200) return __OToolsText("ブルー", "Blue");
        if (r > 200 && g > 200 && b < 50) return __OToolsText("イエロー", "Yellow");
        if (r > 200 && g < 50 && b > 200) return __OToolsText("マゼンタ", "Magenta");
        if (r < 50 && g > 200 && b > 200) return __OToolsText("シアン", "Cyan");
        if (r > 100 && r < 150 && g > 100 && g < 150 && b > 100 && b < 150) return __OToolsText("グレー", "Gray");
    ...
    for (var i = 0; i < selectedLayers.length; i++) {
        ...
        if (convertToEnglishCheckBox.value) {
            ...
            // --- 英語化フラグOFF（日本語デフォルト） ---
            if (layer instanceof CameraLayer) {
                layer.name = __OToolsText("カメラ", "Camera"); 
            } else if (layer instanceof LightLayer) {
                layer.name = __OToolsText("ライト", "Light");
            } else if (layer.matchName === "ADBE Vector Layer") {
                layer.name = __OToolsText("シェイプレイヤー", "Shape Layer");
            } else if (layer.nullLayer) {
                layer.name = "Null";
            } else if (layer.adjustmentLayer) {
                layer.name = __OToolsText("調整レイヤー", "Adjustment Layer");
    ...
    for (var k = 0; k < selectedProjectItems.length; k++) {
        ...
        if (item instanceof FootageItem && item.mainSource instanceof SolidSource) {
            ...
            else if (item.name.indexOf("調整レイヤー") !== -1 || item.name.indexOf("Adjustment Layer") !== -1) {
                item.name = __OToolsText("調整レイヤー", "Adjustment Layer");
            } 
            ...
            // D. 通常の平面（色を判定して「色の名前＋平面」にリネーム）
                ...
                if (cName !== "") {
                    item.name = cName + __OToolsText(" 平面", " Solid");
                } else {
                    item.name = __OToolsText("平面", "Solid");
                }
            }
        }
    }
    ...
    var sizeUpdateLabel = sizeUpdateGroup.add("statictext", undefined, __OToolsText("--名前末尾に_サイズ 追加/更新--", "--Add/Update _Size at the End of Name--"));
    sizeUpdateLabel.helpTip = __OToolsText("選択アイテムの名前にサイズ情報を追加または更新します", "Add or update size information to the names of selected items.");

    var sizeUpdateButton = sizeUpdateGroup.add("button", undefined, __OToolsText("サイズ追加/更新", "Add/Update Size"));
    sizeUpdateButton.helpTip = __OToolsText("選択したアイテムの名前にサイズ情報を追加・更新します", "Add or update size information to the names of selected items.");
    ...
    function addOrUpdateSizeInSelectedItems() {
        ...
        if (selectedItems.length === 0 && !(activeComp instanceof CompItem && activeComp.selectedLayers.length > 0)) {
            alert(__OToolsText("プロジェクトパネルまたはタイムラインパネルでアイテムを選択してください。", "Please select items in the Project panel or Timeline panel."));
            return;
        }
    ...
    var centerLabel = centerGroup.add("statictext", undefined, __OToolsText("--作業画面を中央表示--", "--Center the Composition View--"));
    centerLabel.helpTip = __OToolsText("作業画面を中央に表示します", "Center the composition view in the workspace.");

    var centerButton = centerGroup.add("button", undefined, __OToolsText("中央表示", "Center View"));
    centerButton.helpTip = __OToolsText("アクティブなコンポジションを中央に再描画します", "Redraw the active composition centered in the viewer.");

    centerButton.onClick = function () {
        var comp = app.project.activeItem;
        if (!(comp && comp instanceof CompItem)) {
            alert(__OToolsText("コンポをアクティブにしてください。", "Please activate a composition."));
            return;
        }

        app.beginUndoGroup(__OToolsText("中央表示", "Center View"));
        try {
            ...
            alert(__OToolsText("中央表示に失敗しました:\n", "Failed to center the view:\n") + e.toString());
        }
        app.endUndoGroup();
    };
    ...
    // 見出し
    panel.add("statictext", undefined, __OToolsText("平面をコンポに合わせる", "Composize the plane"));

    // UIブロック
    var fitGrp = panel.add("panel", undefined, __OToolsText("方法選択", "Method Selection"));
    fitGrp.orientation = "column";
    fitGrp.alignChildren = ["left", "top"];

    var fitRb1 = fitGrp.add("radiobutton", undefined, __OToolsText("方法1：スケールで合わせ", "1: Adjust using the scale"));
    var fitRb2 = fitGrp.add("radiobutton", undefined, __OToolsText("方法2：ソースを合わせる", "2: Combine the sauces"));

    // 方法2のオプション（インデント）
    var fitOptGrp = fitGrp.add("group");
    fitOptGrp.margins = [20, 0, 0, 0];
    var fitChkNew = fitOptGrp.add("checkbox", undefined, __OToolsText("新規置換え", "New Replace"));
    fitChkNew.helpTip =
        __OToolsText("チェックを入れると、元の平面ソースを変更せず、新しい平面を作成して置き換えます。\n", "When you check this box, a new plane will be created and used as a replacement without modifying the original plane source.\n") +
        __OToolsText("チェックなしだと、元の平面のサイズを変更します（同じ平面を使っている全レイヤーに影響します）。", "If unchecked, it changes the size of the original plane (affecting all layers using the same plane).");
    ...
    // 実行ボタン
    var fitBtn = panel.add("button", undefined, __OToolsText("実行", "Execute"));
    ...
    fitBtn.onClick = function () {
        var comp = app.project.activeItem;
        if (!(comp && comp instanceof CompItem)) {
            alert(__OToolsText("コンポジションをアクティブにしてください。", "Please activate the composition."));
            return;
        }

        var layers = comp.selectedLayers;
        if (!layers || layers.length === 0) {
            alert(__OToolsText("レイヤーを選択してください。", "Please select a layer."));
            return;
        }
        ...
        try {
            ...
            alert(__OToolsText("Fit to Comp Smart でエラー:\n", "Fit to Comp Smart error:\n") + e.toString());
        } finally {
            app.endUndoGroup();
        }
    };
...
function buildRemoveUI(panel) {
    ...
    var separatorText = panel.add("statictext", undefined, __OToolsText("重複したものを削除します", "Duplicates will be removed"));
    separatorText.alignment = "center";

    var checkButton = panel.add("button", undefined, __OToolsText("調べる", "Start searching in the folder"));
    checkButton.helpTip = __OToolsText("指定した条件に基づいて重複アイテムを検索します", "Searches for duplicate items based on specified criteria");

    var folderGroup = panel.add("group");
    folderGroup.add("statictext", undefined, __OToolsText("選択フォルダ:", "select folder:"));
    var folderPath = folderGroup.add("edittext", undefined, "");
    folderPath.characters = 20;
    folderPath.helpTip = __OToolsText("重複アイテムを検索するフォルダを指定します", "Specify a folder to search for duplicate items");

    var resultText = panel.add("statictext", undefined, __OToolsText("重複数: 0", "Duplication: 0"));
    resultText.helpTip = __OToolsText("検索で検出された重複アイテムの数が表示されます", "Displays the number of duplicate items found in the search");

    var checkBoxGroup = panel.add("panel", undefined, __OToolsText("重複条件", "double condition"));
    checkBoxGroup.orientation = "column";
    checkBoxGroup.alignChildren = ["left", "top"];

    var includeSubfoldersCheckBox = checkBoxGroup.add("checkbox", undefined, __OToolsText("サブフォルダを含める", "Include subfolders"));
    includeSubfoldersCheckBox.value = true;

    var nameCheckBox = checkBoxGroup.add("checkbox", undefined, __OToolsText("名前でチェック", "Check by name"));
    nameCheckBox.value = false;

    var ignoreNumbersCheckBox = checkBoxGroup.add("checkbox", undefined, __OToolsText("名前でチェック(数字_-xX*＊は無視)", "Check by name (ignore numbers)"));
    ignoreNumbersCheckBox.value = true;

    var sizeCheckBox = checkBoxGroup.add("checkbox", undefined, __OToolsText("大きさでチェック", "Check by size"));
    sizeCheckBox.value = true;

    var includeCompsCheckBox = checkBoxGroup.add("checkbox", undefined, __OToolsText("コンポを含める", "include compo"));
    includeCompsCheckBox.value = false;

    var executeButton = panel.add("button", undefined, __OToolsText("実行", "Deleted after summarizing the same"));
    ...
    function checkDuplicates(folder) {
        ...
        resultText.text = __OToolsText("重複数: ", "Duplication: ") + duplicatesCount;
    }
    ...
    function executeConsolidation() {
        ...
        app.endUndoGroup();
        alert(__OToolsText("重複アイテムの整理が完了しました。", "Duplicate items have been sorted out."));
        resultText.text = __OToolsText("重複数: 0", "Duplication: 0");
    }
    ...
    checkButton.onClick = function () {
        if (app.project.selection.length > 0 && app.project.selection[0] instanceof FolderItem) {
            ...
            alert(__OToolsText("プロジェクトパネルでフォルダを選択してください。", "Select a folder in the project panel."));
        }
    };
    ...
    executeButton.onClick = function () {
        if (selectedFolder) {
            ...
            alert(__OToolsText("最初に「調べる」ボタンで重複を確認してください。", "First check for duplicates by clicking on the “Check” button."));
        }
    };
}
...
function buildShakeUI(panel) {
    ...
    var directionPanel = panel.add("panel", undefined, __OToolsText("画ブレ", "Shake Direction"));
    directionPanel.alignChildren = "left";
    var xCheckbox = directionPanel.add("checkbox", undefined, __OToolsText("X方向に揺れる", "Shake in X direction"));
    var yCheckbox = directionPanel.add("checkbox", undefined, __OToolsText("Y方向に揺れる", "Shake in Y direction"));

        // helpTipを追加
        xCheckbox.helpTip = __OToolsText("XとYどちらかにチェックが入ってる場合、その軸だけ揺らします。", "If either X or Y is checked, the shake will be applied along that axis.");
        yCheckbox.helpTip = __OToolsText("XとY両方にチェックが入ってる場合は上下左右ランダムに揺れます。", "If both X and Y are checked, the shake will be applied randomly in all directions.");
    ...
    magnitudeGroup.add("statictext", undefined, __OToolsText("移動幅:", "Magnitude:"));
    var magnitudeInput = magnitudeGroup.add("edittext", undefined, "30"); // 移動幅のデフォルト値を30に設定
    magnitudeInput.characters = 5;
    magnitudeInput.helpTip = __OToolsText("揺れの最大幅をピクセル単位で設定します。", "Set the maximum shake amplitude in pixels.");

    // 横に配置するためにグループを作成
    var durationGroup = panel.add("group");
    durationGroup.add("statictext", undefined, __OToolsText("揺れる時間（フレーム）:", "Duration (frames):"));
    var durationInput = durationGroup.add("edittext", undefined, "20"); // デフォルトは20フレームに設定
    durationInput.characters = 5;
    durationInput.helpTip = __OToolsText("揺れの持続時間をフレーム単位で設定します。", "Set the duration of the shake in frames.");

    var execButton = panel.add("button", undefined, __OToolsText("揺れを適用", "Apply Shake"));
    execButton.helpTip = __OToolsText("指定した設定に基づいて揺れをレイヤーに適用します。", "Apply the shake effect to the selected layers based on the specified settings.");
    ...
    execButton.onClick = function () {
        ...
        if (!isXChecked && !isYChecked) {
            alert(__OToolsText("X方向かY方向のどちらかを選択してください。", "Please select either X or Y direction."));
            return;
        }
        ...
        var layers = comp.selectedLayers;
        if (layers.length === 0) {
            alert(__OToolsText("揺れを適用するレイヤーを選択してください。", "Please select a layer to apply the shake."));
            return;
        }

        app.beginUndoGroup(__OToolsText("揺れを適用", "Apply Shake"));
...
// ★★★★★ Floor All Values (Integer) ★★★★★
    ...
    var floorPanel = panel.add("panel", undefined, __OToolsText("数値の整理", "Organizing the Numbers"));
    floorPanel.alignment = ["fill", "top"];
    
    var floorButton = floorPanel.add("button", undefined, __OToolsText("小数点を切り捨て", "Truncate"));
    floorButton.helpTip = __OToolsText("選択レイヤー内の全プロパティ（トランスフォーム、エフェクト等）の数値を整数にします。", "Convert all property values (transforms, effects, etc.) within the selected layer to integers.");
    ...
    floorButton.onClick = function() {
        ...
        app.beginUndoGroup(__OToolsText("小数点の切り捨て処理", "Truncation of decimal points"));
        ...
        app.endUndoGroup();
        // alert(__OToolsText("処理が完了しました。", "Processing is complete."));
    };
    ...
    var deletePanel = panel.add("panel", undefined, __OToolsText("一括初期化", "Batch Initialization"));
    deletePanel.alignment = ["fill", "top"];
    
    var deleteAllButton = deletePanel.add("button", undefined, __OToolsText("全てのキー・情報を削除", "Delete all keys and information"));
    deleteAllButton.helpTip = __OToolsText("選択レイヤーの全キーフレーム、エクスプレッション、エフェクト、マスクを完全に削除・リセットします。", "Completely delete and reset all keyframes, expressions, effects, and masks on the selected layer.");
    ...
    deleteAllButton.onClick = function() {
        ...
        app.beginUndoGroup(__OToolsText("レイヤーの完全初期化", "Complete Layer Initialization"));
    ...
    var errorFixPanel = panel.add("panel", undefined, __OToolsText("プロジェクト管理", "Project Management"));
    errorFixPanel.alignment = ["fill", "top"];
    errorFixPanel.margins = 15;

    var fixAllErrorsButton = errorFixPanel.add("button", undefined, __OToolsText("エラーエクスプレッションを全て無効化", "Disable Error Expressions"));
    fixAllErrorsButton.size = [200, 40];

    fixAllErrorsButton.onClick = function() {
        var proj = app.project;
        if (proj.numItems === 0) { alert(__OToolsText("プロジェクトが空です。", "The project is empty.")); return; }
        if (!confirm(__OToolsText("プロジェクト内の全コンポをスキャンし、エラーが出ているエクスプレッションを無効化します。", "This will scan ALL compositions in the project and disable expressions with errors. Are you sure?"))) return;

        app.beginUndoGroup(__OToolsText("全プロジェクトのエラーエクスプレッションを無効化", "Disable All Error Expressions"));
        ...
        app.endUndoGroup();
        alert(__OToolsText("完了！\nスキャンしたコンポ: ", "Scan Complete!\nCompositions scanned: ") + compCount + __OToolsText("\n無効化したエラー: ", "\nErrors disabled: ") + errorCount);
    };
