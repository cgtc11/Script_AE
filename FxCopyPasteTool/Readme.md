...
簡単に言うと、あるエフェクトを別の所にある同じエフェクトで数値を同じにしたい。とか、あるエフェクトを別の所にある違うエフェクトと差し替えたい。時に使用する<br>
  但し、現状だと同じレイヤーで同じエフェクトの場合の置換は最後だけ置き換わるので、３つあるなら3回行う必要があり、その後もう一度行えばプロパティも移せるようになってる<br>
  <h2>対応環境</h2>
  <ul>
  ...
  <ul>
    ...
    <li>置換が上手く機能してない。同レイヤーに同じエフェクトが2つ以上ある場合は最後のエフェクトのみ変更するので、複数回変更した後もう一度更新する事で内部プロパティも反映するので注意。</li>
  </ul>
